# Movie API using spring-boot

This movie api is created with the help of spring boot using maven framework. The data is hosted in a mongodb atlas server.

## Setting up database
To set up the projet first we need to create an account in mongo db atlas then create a collection and add the below json data to the collection.
```
[
  {
    "imdbId": "tt3915174",
...
## Adding the DB detials in env file
Create a .env file in the src/main/resources folder and provide the below info in the file.
```
MONGO_DATABASE=""
MONGO_USER=""
MONGO_PASSWORD=""
...
Import the project as a maven project and run as maven clean then maven install to download all the dependencies. Navigate to the MoviebizzApplication.java file and execute - the server will be up.

## API endpoints
Use the below endpoints to perform GET or POST actions.

To retrieve all the movie data
```
localhost:8080/api/movies
```

To retrieve a particular movie using the IMDB id
```
localhost:8080/api/movies/{IMDBid}
```
To send a POST request to add reviews to movies
```
localhost:8080/api/reviews
```
and send the text in below JSON format
```
{
